...
def test_get_fee_impact_absolute(buy_order_details: OrderDetails) -> None:
...
def test_get_fee_impact_percentage_same_asset(buy_order_details: OrderDetails) -> None:
    """Test fee impact calculation with percentage fee in same asset."""
    fee_impact = SpotBalanceEngine._get_fee_impact(buy_order_details)
    assert len(fee_impact) == 1
    # 0.1% of 50000 = 50
    assert fee_impact[buy_order_details.fee.asset] == Decimal("50")
...
def test_get_closing_outflows_with_fee_deduction(
    ...
    assert outflows[0].asset == buy_order_details.fee.asset
    assert outflows[0].amount == Decimal("50")  # 0.1% fee
